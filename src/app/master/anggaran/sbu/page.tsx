...
import DataAnggaranSBU from '@/components/pages/master/anggaran/sbu/list';

const AnggaranSBU = () => {
  const router = useRouter();
  const [dataAnggaranSBU, setDataAnggaranSBU] = useState<any>([]);
  const [search, setSearch] = useState<string>('');

  useEffect(() => {
    fetchDataAnggaranSBU();
  }, []);

  const fetchDataAnggaranSBU = () => {
    let temp: any = [
      {
        id: 1,
        tahun: 2023,
  ...
  return (
    <div className='inisiatif-container'>
      <Breadcrumb pageName="Anggaran SBU" />
      ...
      <div className='body relative'>
        ...
        <div style={gradientStyle}>
          <div className='px-4 flex text-white py-4 space-x-6 font-bold items-center'>
            ...
            <div className='text-title-xsm'>Anggaran SBU</div>
          </div>
        </div>
        <DataAnggaranSBU data={dataAnggaranSBU} />
      </div>
    </div>
  )
}

export default withAuth(AnggaranSBU);