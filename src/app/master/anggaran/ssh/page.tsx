...
import DataAnggaranSSH from '@/components/pages/master/anggaran/ssh/list';

const AnggaranSSH = () => {
  const router = useRouter();
  const [dataAnggaranSSH, setDataAnggaranSSH] = useState<any>([]);
  const [search, setSearch] = useState<string>('');

  useEffect(() => {
    fetchDataAnggaranSSH();
  }, [])

  const fetchDataAnggaranSSH = () => {
    let temp: any = [
      {
        id: 1,
        tahun: 2023,
    ...
    setDataAnggaranSSH(temp);
  };

  const handleAddData = () => router.push('/master/anggaran/ssh/tambah');
  ...
  return (
    <div className='inisiatif-container'>
      <Breadcrumb pageName="Anggaran SSH" />
      ...
      <div className='body relative'>
        ...
        <div style={gradientStyle}>
          <div className='px-4 flex text-white py-4 space-x-6 font-bold items-center'>
            ...
            <div className='text-title-xsm'>Anggaran SSH</div>
          </div>
        </div>
        <DataAnggaranSSH data={dataAnggaranSSH} />
      </div>
    </div>
  )
}

export default withAuth(AnggaranSSH);